...
function StaffDetail({ id, onNav, user }) {
  ...
  return <div className="fw-page wf-page">
    ...
    <WorkforceMetrics items={[
      ...
      { label: 'Effective capabilities', value: canAccess && permissions.data ? formatBusinessNumber(permissionRows.length) : '—', detail: canAccess ? 'Union across active assignments' : 'Outside current access grant', icon: Icons.check },
    ...
    {canAccess ? <DetailSection eyebrow="Effective access" title="Capability union" description="These are the exact backend capability codes produced by active staff account types. The service still checks branch and department scope on every request.">{permissions.pending ? <div className="wf-inline-loading">Calculating effective access…</div> : permissions.error ? <div className="fw-data-note">Effective access could not be calculated for this principal.</div> : <div className="wf-permission-grid">{permissionRows.length ? permissionRows.map((item) => <code key={item}>{item}</code>) : <span>No effective capabilities are recorded.</span>}</div>}</DetailSection> : null}
...
export function StaffPage({ route = 'staff', onNav, user }) {
  ...
  if (segments[1] === 'new') return <StaffEditor onNav={onNav} />;
  if (id && segments[2] === 'edit') return <StaffEditor id={id} onNav={onNav} />;
...
function DepartmentDirectory({ route, onNav, user }) {
  ...
  const visibleBudgets = departments.rows.filter((item) => item.budget != null).length;
  return <div className="fw-page wf-page">
    <WorkspaceHeader eyebrow="Organization design" title="Departments" description="Create and maintain accountable operating units, department heads, branch ownership, active state, and permission-protected budgets." actions={canWrite ? <LinkButton to="departments/new" onNav={onNav} icon={Icons.plus} tone="primary">Create department</LinkButton> : null} />
    <WorkforceMetrics items={[
      ...
      { label: 'Budgets visible on this page', value: departments.rows.some((item) => item.budget != null) ? formatBusinessNumber(visibleBudgets) : 'Protected', detail: 'Values remain permission-protected', icon: Icons.trend },
    ]} />
...
function DepartmentDetail({ id, onNav, user }) {
  const canWrite = canUseCapability(user, 'org:write');
  const access = {
    staff: canUseCapability(user, 'users:read'), teachers: canUseCapability(user, 'teachers:read'), groups: canUseCapability(user, 'cohorts:read'),
  ...
  const toast = useToast();
  const remove = useMutation({ mutationFn: () => httpRequest('DELETE', `/api/v1/org/departments/${id}/`), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['api'] }); toast.success('Department removed.'); onNav('departments'); }, onError: (failure) => toast.danger(userFacingError(failure, { fallback: 'The department could not be removed.' })) });
